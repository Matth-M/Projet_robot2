...
void surveillance_batterie(void){
    ...
    if(flag_moyenne){
        ...
        if (moyenne<seuil_batterie){  //définir le seuil_batterie.
            flag_bat_faible=1;
        ...
        printf("pas assez de valeurs pour faire la moyenne flottante, dernière valeur de Ubat=%d ",Ubat[k]);
    ...
    if(Detecte_i2c(adresse_telecommande_lecture)!=0){
        ...
        if(message_recu[1]=='3')
        {
            ...
            if(flag_bouton_central){
                ...
                arret();
                PORTBbits.RB1 =1; // éteint les capteurs
